...
#             print(f"⚠️ Skipping large file (> {MAX_FILE_SIZE_MB} MB): {file_path}")
#             return []
...
#             return docs
#         else:
#             print(f"⚠️ Skipping empty/non-text file: {file_path}")
#             return []
...
#     for root, _, files in os.walk(DATA_DIR):
#         if not files:
#             print(f"⚠️ Skipping empty folder: {root}")
#             continue
...
#     print(f"📊 Total chunks to embed: {len(all_nodes)}")
...
# ------------------------------
# 3️⃣ Configure Gemini (with big timeouts)
# ------------------------------
Settings.llm = Gemini(
    api_key=google_api_key,
    model="models/gemini-1.5-flash",
...
def read_single_file(file_path):
    ...
    try:
        ...
        if docs and any(doc.text.strip() for doc in docs):
            ...
            print(f"⚠️ Empty/non-text file: {file_path}")
            return []
